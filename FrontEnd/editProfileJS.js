...
//     $tmpInput = $('<textarea name="thisIsHowToDo_'+(inputTag[inputTag.length-1])+'"></textarea>');
...
//                             <input type="file" id="PicTag'+(pictureTag.length-1).toString()+'" name="step_pic_'+(pictureTag.length-1).toString()+'" style="display:none" value="1" />\
//                         </div>\
//                     </div>');
//     // ------------
...
$('#profileImageBtnJa').change(function(){
    if(this.files && this.files[0]){
        var reader = new FileReader();
        reader.onload = function(e){
            $('#profileImg').attr('src', e.target.result);
            $('#profileImg').css('display','initial');
        }
        reader.readAsDataURL(this.files[0]);
    }
});